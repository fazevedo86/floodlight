			...
			if(this.amorphClusterService.isClusterNode(originatingNode)){
				if(msg instanceof MessageContainer){
					...
					if(envelope.queueName.equals(GlobalStateService.STATE_SYNC_QUEUE)){
						...
						try {
							GlobalStateService.class.getDeclaredMethod("handleMessage" + envelope.message.getMessageType().getSimpleName(), ClusterNode.class, IAmorphClusterMessage.class).invoke(this, originatingNode, envelope.message);
